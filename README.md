...
- **Scope: Reusable System Prompts, Not One-Off Requests** — This tool is designed to diagnose system prompts meant to handle variable, sometimes unpredictable future input (e.g., a customer support agent's prompt, an n8n AI node processing live data) — not complete, self-contained one-shot requests. Several diagnostic checks (NO_ERROR_HANDLING, PROMPT_INJECTION_RISK, CONTEXT_OVERLOAD) specifically evaluate robustness against unpredictable future input, which is meaningless for a fully self-contained request with no variable input to handle. A standalone request can still be diagnosed effectively by reframing it as a reusable system prompt — e.g., "explain the theory of relativity to a 12-year-old" becomes a system prompt defining an educator persona, with the specific topic moved into the test input field. This reframing often produces a more robust, reusable prompt as a side effect. See the "Educational explainer prompt" demo example for a worked illustration.
